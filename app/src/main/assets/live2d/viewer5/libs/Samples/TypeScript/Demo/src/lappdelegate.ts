...
export class LAppDelegate {
  ...
  public setDisplayPreset(preset: string): void {
    for (let i = 0; i < this._subdelegates.length; i++) {
      this._subdelegates[i].getLive2DManager()?.setDisplayPreset(preset);
    }
  }

  public applyExpressionByFile(expressionFile: string): void {
    for (let i = 0; i < this._subdelegates.length; i++) {
      this._subdelegates[i].getLive2DManager()?.applyExpressionByFile(expressionFile);
    }
  }

  public playMotionByFile(motionFile: string): void {
    for (let i = 0; i < this._subdelegates.length; i++) {
      this._subdelegates[i].getLive2DManager()?.playMotionByFile(motionFile);
    }
  }

  public lookAtSmooth(x: number, y: number): void {
    for (let i = 0; i < this._subdelegates.length; i++) {
      this._subdelegates[i].getLive2DManager()?.lookAtSmooth(x, y);
    }
  }

  public resetLookAtSmooth(): void {
    for (let i = 0; i < this._subdelegates.length; i++) {
      this._subdelegates[i].getLive2DManager()?.resetLookAtSmooth();
    }
  }

  public setTalkState(talking: boolean): void {
    for (let i = 0; i < this._subdelegates.length; i++) {
      this._subdelegates[i].getLive2DManager()?.setTalkState(talking);
    }
  }

  public playTapReactionSmooth(x: number, y: number): void {
    for (let i = 0; i < this._subdelegates.length; i++) {
      this._subdelegates[i].getLive2DManager()?.playTapReactionSmooth(x, y);
    }
  }
